...
const { loader, showNav, showCart, pageMeta } = storeToRefs(layoutStore)
const { toggleNav, setShowNav, setShowCart, toggleCart } = layoutStore

const cartStore = useCartStore()
const { cart, discount, subtotal, total } = storeToRefs(cartStore)
const { cartAction } = cartStore
</script>

<template>
  <section class="main">
    <SideNav :showNav="showNav" @toggle="toggleNav" />
    <CartDrawer
      :showCart="showCart"
      :cart="cart"
      :discount="discount"
...
.header-lg .lg-menu .nav-item .router-link-exact-active {
  color: #e24585;
}
